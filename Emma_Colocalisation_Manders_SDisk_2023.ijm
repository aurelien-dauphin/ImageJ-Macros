 ...
 * Use Jacop plugin to get Pearson, Manders  
 * 
...
//--------------------Make a composite with nucleus's mask, ch2 and ch3 cropped and renamed roi_1, roi_2...--------
	if (chN==1) 
	{
		if (endsWith(listeFichiers[l], "_ch_0_nucleus_mask.tif")) 
		{
			...
			nome=getTitle();
			nomeim=substring(nome, 0 , nome.length-22);	
			print(" ");
			print(" ");
		...
		// ---- make the composite
		  	...
		  	selectWindow("wch2_raw");
		  		close();
		// ---- Open the roi
			roiManager("Open", dir+nomeim+"_w3405-QUAD_RoiSet.zip");
...
//------------------( Call Function )to assess colocalisation i----------	
			...
			*/
			...
			run("JACoP ", "imga=C1-roi_"+i+" imgb=C2-roi_"+i+" thra="+lowerA+" thrb="+lowerB+" pearson mm");
			// objdist=2120000-2129400-"+colocvolumexy+"-"+colocvolumez+"-true-true-true 			//******
		log1=getInfo("log");
		log2=substring(log1, lastIndexOf(log1, "r=")+2, lastIndexOf(log1, "Manders' Coefficients (original):")-2);
				log3=substring(log1, lastIndexOf(log1, "M1=")+3, lastIndexOf(log1, "M2=")-31);
						log4=substring(log1, lastIndexOf(log1, "M2=")+3, lastIndexOf(log1, "(fraction")-1);
					...
					print(log2+"#"+log3+"#"+log4); //print(log2+"#"+log3+"#"+log4+"#"+log5+"#"+log6+"#"+log7+"#"+log8);			
...
run("New... ", "name="+Titre+" type=Table");
 				...
 				header="Name"+"\t Pearson \t M1 \t M2 \t ThresA \t ThresB";
...
//print("max data = "+max+"\t nfichiers ="+nFichiers);
	...
	Rez=nom[j]+"\t"+pearson[j]+"\t"+m1[j]+"\t"+m2[j]+"\t"+lowA[j]+"\t"+lowB[j]+"\t";
